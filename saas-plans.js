...
(function() {
  ...
  window.SaasPlans = {
    ...
    showUpgradeModal(feature) {
      ...
      const nome = featureNames[feature] || feature;

      // Usa UIService se disponível, senão alert simples
      if (window.UIService?.showToast) {
        UIService.showToast(
          '🔒 Recurso bloqueado',
          `"${nome}" não está disponível no plano ${SaasPlans.getInfo().planoNome}. Faça upgrade para acessar.`,
          'warning'
        );
      } else {
        alert(`🔒 "${nome}" não está disponível no seu plano atual.\n\nFaça upgrade para acessar este recurso.`);
      }
    },
